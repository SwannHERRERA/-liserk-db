...
use crate::utils::to_uuid::to_uuid;
...
async fn query_cats(
    ...
    // INFO I have remove an option filter by removing the database

    let (cats, count) = Cat::find_and_count(doc! { "user": &user.id }).await?;
